...
void ChatInterfaceCell::setString(ChatInfoObject& object)
{
    ...
    CCLabelTTF* tmp = CCLabelTTF::create(m_object->content.c_str(), "Arial", ttfSize);
    row = (unsigned int)ceilf(tmp->getContentSize().width/lenght);
    height = MAX(height, row * (tmp->getContentSize().height));
    m_contentTTF = CCLabelTTF::create(m_object->content.c_str(), "Arial", ttfSize, Size(lenght, height+1), kCCTextAlignmentLeft);
